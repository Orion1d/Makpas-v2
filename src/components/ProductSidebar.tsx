...
export function ProductSidebar({
  ...
  const MobileFilterBar = () => <div className="md:hidden w-full space-y-4 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-4 border-b sticky top-14 z-20">
      <div className="relative">
        ...
        <Input placeholder={t('search_placeholder')} value={searchQuery} onChange={e => onSearchChange(e.target.value)} className="pl-8" />
      </div>
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {sortedGroups.map(group => <button key={group} onClick={() => onGroupChange(group)} className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${activeGroup === group ? "bg-secondary text-secondary-foreground" : "bg-muted hover:bg-muted/80"}`}>
            {getGroupDisplayName(group)}
          </button>)}
      </div>
      <Button variant="outline" className="w-full flex items-center gap-2" onClick={handleCatalogDownload}>
      ...
      <Sidebar className={`hidden md:block sticky top-16 h-[calc(100vh-4rem)] ${className}`}>
        <SidebarContent>
          ...
          <SidebarGroup>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {sortedGroups.map(group => <SidebarMenuItem key={group}>
                    <SidebarMenuButton onClick={() => onGroupChange(group)} className={`w-full capitalize ${activeGroup === group ? "bg-secondary text-secondary-foreground" : ""}`}>
          ...
          <SidebarGroup className="my-[240px] py-[90px]">
