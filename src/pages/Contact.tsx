            ...
            <div className="relative z-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="relative">
                  <div className="relative border-b-2 border-gray-300 dark:border-gray-500 transition-colors duration-250 focus-within:border-safety-orange group">
                    <input
                      ...
                      onBlur={handleBlur}
                      className="block w-full px-0 py-3 text-gray-900 dark:text-white bg-transparent appearance-none focus:outline-none peer"
                      placeholder=" "
                      required
                    />
                    <label
                      htmlFor="name"
                      className={`absolute text-gray-500 duration-250 transform -translate-y-8 scale-75 top-3 origin-[0] 
                      ${(focused === 'name' || formData.name) ? 
                        '-translate-y-8 scale-75 text-sm text-safety-orange' : 
                    ...
                    {getFieldStatus('name') === 'valid' && (
                      <CheckCircle className="absolute right-0 top-3 text-green-500 h-5 w-5" />
                    )}
                    {getFieldStatus('name') === 'invalid' && (
                      <X className="absolute right-0 top-3 text-red-500 h-5 w-5" />
                    )}
                  </div>
                  {getFieldStatus('name') === 'invalid' && touched.name && (
                    <p className="mt-1 text-sm text-red-500">{t('name_required') || 'Name is required (min 3 characters)'}</p>
                  )}
                </div>
                ...
                <div className="relative">
                  ...
                  {getFieldStatus('email') === 'invalid' && touched.email && (
                    <p className="mt-1 text-sm text-red-500">{t('valid_email_required') || 'A valid email is required'}</p>
                  )}
                </div>
                ...
                <div className="relative">
                  <div className="relative border-b-2 border-gray-300 dark:border-gray-500 transition-colors duration-250 focus-within:border-safety-orange group">
                    <textarea
                      ...
                      onBlur={handleBlur}
                      className="block w-full px-0 py-3 text-gray-900 dark:text-white bg-transparent appearance-none focus:outline-none peer resize-none"
                      placeholder=" "
                      required
                    />
                    <label
                  ...
                  {getFieldStatus('message') === 'invalid' && touched.message && (
                    <p className="mt-1 text-sm text-red-500">{t('message_required') || 'Message is required (min 3 characters)'}</p>
                  )}
                </div>
                ...
                <Button 
                  type="submit" 
                  variant="accent"
                  className="w-full mt-8 relative transition-all duration-250 bg-gradient-to-r from-safety-orange to-safety-orange/80 hover:to-safety-orange/90"
          ...
          >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a 
                      ...
                      className="text-gray-600 dark:text-gray-300 hover:text-safety-orange dark:hover:text-safety-orange transition-colors"
                    >
                      {t('contact_email')}
                    </a>
                  </div>
            ...
            <Card className="bg-secondary shadow-md border-none transition-transform duration-250 hover:scale-[1.02]">
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold text-lg">{t('emergency_contact') || 'Emergency Contact'}</h3>
                  <p className="text-white text-sm">{t('emergency_hours') || 'Available 24/7 for urgent issues'}</p>
                  <a href="tel:+905551234567" className="text-white font-bold underline mt-1 inline-block">
                    +90 555 123 45 67
                  </a>
                </div>
              </CardContent>
        ...
        <div className="bg-white dark:bg-primary/90 p-4 md:p-6 rounded-lg shadow-md py-[10px] relative overflow-hidden group border-2 border-gray-200 dark:border-gray-700 transition-all duration-250 hover:border-safety-orange hover:shadow-lg">
          ...
          <div className="w-full relative aspect-video overflow-hidden rounded-lg">
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-250 flex items-center justify-center opacity-0 group-hover:opacity-100">
              <a 
                href="https://www.google.com/maps?ll=40.239897,28.943823&z=17&t=h&hl=tr&gl=ES&mapclient=embed&cid=3140769231945303382" 
                target="_blank" 
                rel="noopener noreferrer"
                className="px-6 py-3 bg-safety-orange text-white rounded-lg shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-250"
              >
